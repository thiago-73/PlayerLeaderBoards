...
public class PlayerLeaderBoards extends JavaPlugin implements Listener, CommandExecutor {
    ...
    public void onEnable() {
        ...
        for (String cmd : Arrays.asList("spawnkillshologram", "spawndeathshologram", "spawnplaytimehologram",
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        statManagers.get(StatType.DEATHS).add(event.getEntity(), 1);
        hologramManager.updateStat(StatType.DEATHS);
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (name) {
            ...
            case "spawndeathshologram":
                return spawnCommand((Player) sender, StatType.DEATHS);
    ...
    private boolean leaderboardCommand(CommandSender sender, String[] args) {
        if (args.length < 1) {
            sender.sendMessage(ChatColor.YELLOW + "Uso: /leaderboard <kills|deaths|playtime|messages|blocksmined|distancewalked>");
            return true;
        }
